function Clear-CurrentUserTemp {
    ...
    .DESCRIPTION
        ...
        deletion request. Ancestor directory handles remain held through candidate
        mutation so a validated parent cannot be renamed or replaced by a reparse
        point during the operation. The handle requests DELETE and
        FILE_READ_ATTRIBUTES only; it does not read file contents. Retention and
        logical byte counts use metadata observed from that deletion handle.
        Data writers and renames are blocked while it is held, but attribute-only
        timestamp changes can still occur. The retention check and deletion are
        not an atomic operation. Directory removal is opt-in, non-recursive, deepest-first,
