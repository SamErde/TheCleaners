...
Source and package evidence was collected at commit `e6322a5c0f32f8a957f595a5a06bf1eac25dfba2`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 81.76% (`950/1162` commands) with no skipped tests at `e6322a5`, while hosted CI and remaining unexecuted failure branches still require evidence.
...
| Typed stale-profile output | `src/Tests/Unit/ProfileOutput.Tests.ps1`: typed fields, unknown dates, optional size, exclusions, queued-directory reparse replacement fail-closed behavior, no host output, and orphan-helper retirement. |
