...
Source and package evidence was collected at commit `e810091dce8b103e5a5821c68e9090514404a1b8`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| PS7 current suite | Pester `5.9.1`; `177/177` passed; `0` failed, skipped, or not run; report runtime `7.6.6 Core`; no coverage instrumentation was enabled for this run; tested commit `e810091dce8b103e5a5821c68e9090514404a1b8`. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `177/177` unit tests passed; `0` failed, skipped, or not run; `81.6%` of `1,277` analyzed commands executed; report commit `e810091dce8b103e5a5821c68e9090514404a1b8`. The reusable build task preloads the CodeDOM-generated native helper before Pester creates worker runspaces, so the complete native suite ran. | Hosted PS5.1 acceptance still requires its CI run URL. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 81.6% of `1,277` analyzed commands with no skipped tests at `e810091`, while hosted CI and remaining unexecuted failure branches still require evidence.
