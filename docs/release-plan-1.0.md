...
Source and package evidence was collected at commit `f814404c2948d40daa818122613a9b9336544b74`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| PS7 current suite | Pester `5.9.1`; `150/150` passed; `0` failed, skipped, or not run; report runtime `7.6.6 Core`; no coverage instrumentation was enabled for this run. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `150/150` unit tests passed; `0` failed, skipped, or not run; `80.20%` (`798/995` commands executed/analyzed); report commit `f814404c2948d40daa818122613a9b9336544b74`. The test harness preloaded the CodeDOM-generated native helper before Pester created worker runspaces, so the complete native suite ran. | Hosted PS5.1 acceptance still requires its CI run URL. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 80.20% with no skipped tests at `f814404`, while hosted CI and remaining unexecuted failure branches still require evidence.
