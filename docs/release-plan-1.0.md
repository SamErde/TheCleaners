...
| PS7 current suite | Pester `5.9.1`; PS `7.6.6 Core`; `193/193` passed; `0` failed, skipped, or not run; `83.17%` of `1,503` analyzed commands executed; tested code commit `f6b10b6ae2be30e03b9bf7f6edf6cf270b8c88fe`. Coverage used Pester's profiler mode (`UseBreakpoints=false`), now configured by the build task; the desktop execution host rejects breakpoint mode with `Debugging is not supported on remote sessions`. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `193/193` unit tests passed; `0` failed, skipped, or not run; `83.17%` of `1,503` analyzed commands executed; tested code commit `f6b10b6ae2be30e03b9bf7f6edf6cf270b8c88fe`. The native helper was preloaded before Pester created worker runspaces, and coverage used the same profiler mode configured by the build task. | Hosted PS5.1 acceptance still requires its CI run URL. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS7.6.6 and PS5.1 each measured `83.17%` of `1,503` analyzed commands with no failed, skipped, or not-run tests at `f6b10b6`, while hosted CI and remaining unexecuted failure branches still require evidence.
