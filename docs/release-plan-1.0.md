...
Source and package evidence was collected at commit `430ac45908c9c01b898ae0b2971e3089311f3794`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| PS7 current suite | Pester `5.9.1`; `165/165` passed; `0` failed, skipped, or not run; report runtime `7.6.6 Core`; no coverage instrumentation was enabled for this run; tested commit `430ac45908c9c01b898ae0b2971e3089311f3794`. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `165/165` unit tests passed; `0` failed, skipped, or not run; `81.51%` (`930/1141` commands executed/analyzed); report commit `430ac45908c9c01b898ae0b2971e3089311f3794`. The test harness preloaded the CodeDOM-generated native helper before Pester created worker runspaces, so the complete native suite ran. | Hosted PS5.1 acceptance still requires its CI run URL. |
| ACL and native deletion fixture | PS7.6.6 fixture `TheCleaners-Acl-13502cbd-a074-413c-a302-4d148349364c` and PS5.1.26100.9444 fixture `TheCleaners-Acl-ba7781bd-e9a9-4238-8b54-af101992bc55` both used C: NTFS, non-elevated. Each exact candidate list contained `old-readable.tmp` and `old-delete-without-read.tmp`; expected/after counts were `2 -> 0`; `ReadWasDenied=true`; explicit delete allow mask `65536`; read deny mask `1`; `FilesRemoved=2`; `BytesReclaimed=6`; result status `Completed`; skips/failures/error IDs `0`; outcome reconciliation and acceptance `true`; protected paths `0`. | This does not replace elevated/ReFS/real server lab acceptance. |
| Product preview probe | Host `BN7FXL4`, recorded `2026-09-15T11:24:16.0997565Z`, PS7.6.6 Core, Windows 11 Enterprise `10.0.26200`, non-elevated, `DeletionEnabled=false`. IIS registry/product absent, `WebAdministration=null`, W3SVC and WAS `NotInstalled`, protected paths `[]`, candidates unavailable, preview `Unavailable` with `IISDiscoveryUnavailable,Clear-OldIISLog`. Exchange registry/product absent, management commands `[]`, all four checked Exchange services `NotInstalled`, protected paths `[]`, candidate counts unavailable, preview `Unavailable` with `ExchangeRegistryDiscoveryFailed,Clear-OldExchangeLog`. | Disposable IIS and Exchange product/build labs, exact candidate lists, before/after counts, protected paths, and service-health evidence remain open; absence on this workstation is not acceptance. |
| Package and clean install | PS7.6.6 build stages completed with Pester `5.9.1`, analyzer/formatting gates, `4/4` integration tests, deterministic archive `TheCleaners_0.0.15.zip`, archive SHA-256 `cd9de3fcf2b4274b9e71f9d8ca272608c90cc92ecc391358231db5b9b4261784`, exact manifest/file-hash verification, and clean-install probes across every available local host; tested commit `430ac45908c9c01b898ae0b2971e3089311f3794`. | The package remains prerelease `0.0.15-beta`; hosted matrix evidence and maintainer acceptance remain open. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 81.51% (`930/1141` commands) with no skipped tests at `430ac45`, while hosted CI and remaining unexecuted failure branches still require evidence.
...
| Concurrent writer, profile identity, and unavailable-product safety | `src/Tests/Unit/CleanupBehavior.Tests.ps1`: deletion without write sharing, candidate file and planned-directory verbose paths; `src/Tests/Unit/TempCandidateSafety.Tests.ps1`: opened-handle last-write metadata and directory replacement lock; `src/Tests/Unit/ProfileOutput.Tests.ps1`: complete service/application-pool SID exclusions and reparse-point ancestor rejection; `src/Tests/Unit/PreviewCommandSafety.Tests.ps1`: unavailable IIS product and protected roots report structured failed results. |
