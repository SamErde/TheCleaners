...
Source and package evidence was collected at commit `c8f331dd3d0e813e2ad5e2f637609c8a16383611`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| PS7 current suite | Pester `5.9.1`; `183/183` passed; `0` failed, skipped, or not run; report runtime `7.6.6 Core`; no coverage instrumentation was enabled for this run; tested commit `c8f331dd3d0e813e2ad5e2f637609c8a16383611`. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `183/183` unit tests passed; `0` failed, skipped, or not run; `83.49%` of `1,369` analyzed commands executed; report commit `c8f331dd3d0e813e2ad5e2f637609c8a16383611`. The reusable build `Test` task preloads the CodeDOM-generated native helper before Pester creates worker runspaces, so the complete native suite ran. | Hosted PS5.1 acceptance still requires its CI run URL. |
| ACL and native deletion fixture | PS7.6.6 fixture `TheCleaners-Acl-4e6f7f29-72e2-4481-8be6-ac2b49e95548` and PS5.1.26100.9444 fixture `TheCleaners-Acl-9f62ad36-b1e1-4a05-9a63-6e2d8514b434` both used C: NTFS, non-elevated. Each exact candidate list contained `old-readable.tmp` and `old-delete-without-read.tmp`; expected/after counts were `2 -> 0`; `ReadWasDenied=true`; explicit delete allow mask `65536`; read deny mask `1`; `FilesRemoved=2`; `BytesReclaimed=6`; result status `Completed`; skips/failures/error IDs `0`; outcome reconciliation and acceptance `true`; protected paths `0`. | This does not replace elevated/ReFS/real server lab acceptance. |
| Product preview probe | Host `BN7FXL4`, recorded `2026-09-15T15:47:06.9918423Z`, PS7.6.6 Core, Windows 11 Enterprise `10.0.26200`, non-elevated, `DeletionEnabled=false`. IIS registry/product absent, `WebAdministration=null`, W3SVC and WAS `NotInstalled`, protected paths `[]`, candidates unavailable, preview `Unavailable` with `IISDiscoveryUnavailable,Clear-OldIISLog`. Exchange registry/product absent, management commands `[]`, all four checked Exchange services `NotInstalled`, protected paths `[]`, candidate counts unavailable, preview `Unavailable` with `ExchangeRegistryDiscoveryFailed,Clear-OldExchangeLog`; registry and service reads now distinguish missing products from access/provider failures. | Disposable IIS and Exchange product/build labs, exact candidate lists, before/after counts, protected paths, and service-health evidence remain open; absence on this workstation is not acceptance. |
| Package and clean install | PS7.6.6 build stages completed with Pester `5.9.1`, analyzer/formatting gates, `4/4` integration tests, deterministic archive `TheCleaners_0.0.15.zip`, archive SHA-256 `B8BD55B133CD39C9A5B4C4C8B0F10C173CE88E6DDDF4271F6B3019DC392179C5`, exact manifest/file-hash verification including staged-file rehashes, and clean-install probes across every available local host; tested commit `c8f331dd3d0e813e2ad5e2f637609c8a16383611`. | The package remains prerelease `0.0.15-beta`; hosted matrix evidence and maintainer acceptance remain open. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 83.49% of `1,369` analyzed commands with no skipped tests at `c8f331d`, while hosted CI and remaining unexecuted failure branches still require evidence.
...
| Typed stale-profile output | `src/Tests/Unit/ProfileOutput.Tests.ps1`: typed fields, unknown dates, optional size, exclusions, queued-directory reparse replacement fail-closed behavior, native directory identity lock during enumeration, no host output, and orphan-helper retirement. |
