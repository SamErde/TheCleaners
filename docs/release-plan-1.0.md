...
Source and package evidence was collected at commit `85aed5b21d656a7de76bf40fd1e192dd98b0b471`; the latest review-fix code, test, and package evidence was collected at commit `db23cdaf978391c669eb07d806d879b88d1d0605`. The ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| PS7 current suite | Pester `5.9.1`; PS `7.6.6 Core`; `195/195` passed; `0` failed, skipped, or not run; `83.59%` of `1,530` analyzed commands executed; tested code commit `db23cdaf978391c669eb07d806d879b88d1d0605`. Coverage used Pester's profiler mode (`UseBreakpoints=false`), now configured by the build task; the desktop execution host rejects breakpoint mode with `Debugging is not supported on remote sessions`. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `195/195` unit tests passed; `0` failed, skipped, or not run; `83.59%` of `1,530` analyzed commands executed; tested code commit `db23cdaf978391c669eb07d806d879b88d1d0605`. The native helper was preloaded before Pester created worker runspaces, and coverage used the same profiler mode configured by the build task. | Hosted PS5.1 acceptance still requires its CI run URL. |
| ACL and native deletion fixture | PS7.6.6 fixture `TheCleaners-Acl-4a22016a-43d6-4fa8-8e7e-62f8ec0323b9` and PS5.1.26100.9444 fixture `TheCleaners-Acl-627188be-d9bb-42b6-93cb-6ec5276ee052` both used C: NTFS, non-elevated. Each exact candidate list contained `old-readable.tmp` and `old-delete-without-read.tmp`; expected/after counts were `2 -> 0`; `ReadWasDenied=true`; explicit delete allow mask `65536`; read deny mask `1`; `FilesRemoved=2`; `BytesReclaimed=6`; result status `Completed`; skips/failures/error IDs `0`; outcome reconciliation and acceptance `true`; protected paths `0`. | This does not replace elevated/ReFS/real server lab acceptance. |
| Product preview probe | Host `BN7FXL4`, PS7.6.6 Core recorded `2026-09-15T17:10:58.9228988Z` and PS5.1.26100.9444 Desktop recorded `2026-09-15T17:11:23.0200083Z`; Windows 11 Enterprise `10.0.26200`, non-elevated, `DeletionEnabled=false` in both runs. IIS product build `null`, registry/product absent, `WebAdministration=null`, W3SVC and WAS `NotInstalled`, protected paths `[]`, candidate lists before/after `[]`, counts unavailable, preview `Unavailable` with `IISDiscoveryUnavailable,Clear-OldIISLog`. Exchange product build `null`, registry/product absent, management commands `[]`, all four checked Exchange services `NotInstalled`, protected paths `[]`, candidate lists before/after `[]`, counts unavailable, preview `Unavailable` with `ExchangeRegistryDiscoveryFailed,Clear-OldExchangeLog`; registry and service reads distinguish missing products from access/provider failures. | Disposable IIS and Exchange product/build labs, exact candidate lists, before/after counts, protected paths, and service-health evidence remain open; absence on this workstation is not acceptance. |
| Package and clean install | At code commit `db23cdaf978391c669eb07d806d879b88d1d0605`, PS7.6.6 and PS5.1.26100.9444 each completed the full `BuildNoIntegration` package task successfully with 19-file manifests and no BOM in staged public scripts. The PS7 archive `TheCleaners_0.0.15.zip` has SHA-256 `438ce78884b7774b080c0518dd05035090d9d1526ebdc646f6a85bb5cc39e5e1`; the PS5.1 .NET Framework compressor produced `e6413fd15477f594a3c8b28e53ef76e63a009e73e8b5752aa0105e276bcf6877`. Each runtime then passed `4/4` integration tests, including exact-manifest/sidecar checks, quiet import, preview locks, extraction, and clean-install probes across every available local host. | The package remains prerelease `0.0.15-beta`; hosted matrix evidence and maintainer acceptance remain open. |
...
| TC-004 | Temp cleaners, opt-in directory pruning, WhatIf/Confirm, locked-file/race handling, privilege/root preflight | Draft implementation includes same-handle identity checks, only candidate-ancestor and planned-directory handles retained through mutation, skipped or failed candidates disqualifying their parent and ancestors before pruning, stable directory handles during mutation-enabled traversal, root preflight, lock/race fixtures, hard-link/reparse tests, actual-root/privilege checks, queued-directory replacement fail-closed handling, ACL evidence, and a fresh-process interactive Y/N confirmation probe. ReFS/elevated/server acceptance and hosted CI remain open. |
| TC-005 | IIS path discovery, environment expansion, per-format allowlist, deduplication, inline deletion, and server validation | Preview lock, read-only web and FTP discovery from separate configured roots, configured FTP logging-format propagation, fallback FTP-family classification, documented W3C `u_exYYMMDD.log` rollover matching, validated-root normalization/deduplication, protected-path checks, format allowlists, and dependency restoration are implemented in draft. The local probe recorded IIS absence only; product/version and server lab validation remain open; removal stays disabled. |
...
- [x] Add complete 5.1 plus supported 7 LTS/current package/install tests, not just source unit tests; local PS5.1/PS7.6.6 build, package, and install evidence is recorded above, while hosted 7 LTS/current and CI evidence remain open.
- [x] Re-enable the ShouldProcess analyzer rule; no broad suppression is used in the draft source.
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS7.6.6 and PS5.1 each measured `83.59%` of `1,530` analyzed commands with no failed, skipped, or not-run tests at `db23cdaf978391c669eb07d806d879b88d1d0605`, while hosted CI and remaining unexecuted failure branches still require evidence.
...
| Profile ancestors remain stable through queued size traversal | `src/Tests/Unit/ProfileOutput.Tests.ps1`: `holds profile ancestors while traversing queued directories` attempts a profile-root rename after a child is queued; the handle/revalidation boundary blocks or detects replacement before `SizeStatus=Available`, while each queued directory also carries and rechecks its discovery-time native identity. |
...
| Review regressions closed in the final patch | `src/Tests/Unit/ExchangePreviewEdgeCases.Tests.ps1` and `src/Tests/Unit/IISDiscoverySafety.Tests.ps1`: extended-length namespaces remain on provider traversal paths while comparison/result paths are normalized, IIS FTP discovery failures stay independent from valid web-root candidates and return per-FTP structured failures, and the MkDocs deployment rebuild uses strict validation. `src/Tests/Unit/CoverageEdges.Tests.ps1` verifies stable handles are released for branches without mutation candidates; `src/Tests/Unit/RuntimeSafety.Tests.ps1` covers one explicit `Y` and one explicit `N` without nested prompts; profile sizing retains only stable profile ancestors while queued child handles are transient and revalidated. |
