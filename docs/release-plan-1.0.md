...
Source and package evidence was collected at commit `134bbecc037b470533d5b3c6731b67a20c5f36cb`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| PS7 current suite | Pester `5.9.1`; `152/152` passed; `0` failed, skipped, or not run; report runtime `7.6.6 Core`; no coverage instrumentation was enabled for this run. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `152/152` unit tests passed; `0` failed, skipped, or not run; `82.10%` (`835/1017` commands executed/analyzed); report commit `134bbecc037b470533d5b3c6731b67a20c5f36cb`. The test harness preloaded the CodeDOM-generated native helper before Pester created worker runspaces, so the complete native suite ran. | Hosted PS5.1 acceptance still requires its CI run URL. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 82.10% (`835/1017` commands) with no skipped tests at `134bbec`, while hosted CI and remaining unexecuted failure branches still require evidence.
