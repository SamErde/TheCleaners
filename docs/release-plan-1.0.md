...
Source and package evidence was collected at commit `b922a6bb7cce2b2acc54027a1cfb794eb7df8531`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `149/149` unit tests passed; `0` failed, skipped, or not run; `80.20%` (`798/995` commands executed/analyzed); report commit `b922a6bb7cce2b2acc54027a1cfb794eb7df8531`. The test harness preloaded the CodeDOM-generated native helper before Pester created worker runspaces, so the complete native suite ran. | Hosted PS5.1 acceptance still requires its CI run URL. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 80.20% with no skipped tests at `b922a6b`, while hosted CI and remaining unexecuted failure branches still require evidence.
