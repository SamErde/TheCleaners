...
Source and package evidence was collected at commit `85aed5b21d656a7de76bf40fd1e192dd98b0b471`; the ledger update itself is documentation-only and this is not a merged commit or a hosted CI run. No real Windows, IIS, or Exchange cleanup was run, and no release tag or Gallery publication was performed.
...
| PS7 current suite | Pester `5.9.1`; `187/187` passed; `0` failed, skipped, or not run; report runtime `7.6.6 Core`; no coverage instrumentation was enabled for this run; tested commit `85aed5b21d656a7de76bf40fd1e192dd98b0b471`. The repository coverage task discovered all `187` tests, but this desktop wrapper rejected Pester's debugger breakpoint with `Debugging is not supported on remote sessions`, so no PS7 coverage percentage is claimed. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `187/187` unit tests passed; `0` failed, skipped, or not run; `83.91%` of `1,442` analyzed commands executed; report commit `85aed5b21d656a7de76bf40fd1e192dd98b0b471`. The reusable build `Test` task preloads the CodeDOM-generated native helper before Pester creates worker runspaces, so the complete native suite ran. | Hosted PS5.1 acceptance still requires its CI run URL. |
...
| Package and clean install | At commit `85aed5b21d656a7de76bf40fd1e192dd98b0b471`, PS7.6.6 and PS5.1.26100.9444 both completed `HelpLocal,Build,Archive` successfully with 19-file manifests and no BOM in staged public scripts. The per-file manifest lengths/SHA-256 values matched across hosts. The tested PS7 archive is `TheCleaners_0.0.15.zip` with SHA-256 `FC41B6D37BD04A410BC822B45D133BBD2DA9D3870BC3C629715BF2D3C69F7553`; the PS5.1 .NET Framework compressor produced `B480293B2F3616F1BA7267B11231A1BB9EFDCD862468C9A2AADD02FB95D7B9B3` for the same 19 file records. PS7 and PS5.1 each passed `4/4` integration tests, including exact-manifest/sidecar checks, quiet import, preview locks, extraction, and clean-install probes across every available local host. | The package remains prerelease `0.0.15-beta`; hosted matrix evidence and maintainer acceptance remain open. |
...
| TC-004 | Temp cleaners, opt-in directory pruning, WhatIf/Confirm, locked-file/race handling, privilege/root preflight | Draft implementation includes same-handle identity checks, ancestor handles held through mutation, skipped child handles retained through ancestor pruning, stable directory handles during mutation-enabled traversal, root preflight, lock/race fixtures, hard-link/reparse tests, actual-root/privilege checks, queued-directory replacement fail-closed handling, and ACL evidence. ReFS/elevated/server acceptance, interactive confirmation acceptance, and hosted CI remain open. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS5.1 measured 83.91% of `1,442` analyzed commands with no skipped tests at `85aed5b`, while hosted CI and remaining unexecuted failure branches still require evidence.
...
PR #31 adds parameterized filesystem tests for both temp commands, inclusive cutoff, literal names, junction exclusion, directory scope, WhatIf, partial failure, ErrorAction Stop, failed enumeration, a candidate replaced by a directory, a candidate disappearing before deletion, and skipped-child handle retention through ancestor pruning. It adds IIS and Exchange no-mutation/guard/root-type tests, configured FTP-format propagation, mixed-separator namespace rejection, export/help/alias tests, source import checks, and a fresh-process built-package probe.
...
| Skipped temp children remain protected during ancestor pruning | `src/Tests/Unit/CleanupBehavior.Tests.ps1`: `retains skipped child handles through ancestor pruning`, parameterized for both temp commands, adds a recent child file after discovery, verifies the child handle blocks a concurrent rename, and keeps the directly touched ancestor. |
