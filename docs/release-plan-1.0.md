...
| PS7 current suite | Pester `5.9.1`; PS `7.6.6 Core`; `189/189` passed; `0` failed, skipped, or not run; `83.87%` of `1,488` analyzed commands executed; tested code commit `6d53f38575894956ed2771215c1ac79006299544`. Coverage used Pester's profiler mode (`UseBreakpoints=false`) because the repository wrapper's normal debugger-breakpoint mode is rejected by this desktop execution host as `Debugging is not supported on remote sessions`. | Hosted CI still needs the configured supported 7 LTS and current-stable run URLs. |
| Windows PowerShell compatibility and coverage | Pester `5.9.1`; PS `5.1.26100.9444` Desktop; `189/189` unit tests passed; `0` failed, skipped, or not run; `83.87%` of `1,488` analyzed commands executed; tested code commit `6d53f38575894956ed2771215c1ac79006299544`. The native helper was preloaded before Pester created worker runspaces, and coverage used the same profiler mode. | Hosted PS5.1 acceptance still requires its CI run URL. |
...
| Package and clean install | At code commit `6d53f38575894956ed2771215c1ac79006299544`, PS7.6.6 and PS5.1.26100.9444 both completed `HelpLocal,Build,Archive` successfully with 19-file manifests and no BOM in staged public scripts. The per-file manifest lengths/SHA-256 values matched across hosts (`DifferentManifestRecords=0`). The PS7 archive is `TheCleaners_0.0.15.zip` with SHA-256 `2FAF2FFD3670AF09A18791F3B0F779C7E295A01EAD0F176287D39B06AC9CC8BE`; the PS5.1 .NET Framework compressor produced `80E3FA551FBDD8CC9AB1042D49D4355B18968F812FF6E2ADAC17EC57679B549D` for the same 19 file records. PS7 and PS5.1 each passed `4/4` integration tests, including exact-manifest/sidecar checks, quiet import, preview locks, extraction, and clean-install probes across every available local host. | The package remains prerelease `0.0.15-beta`; hosted matrix evidence and maintainer acceptance remain open. |
...
- [x] Meet at least 80% overall coverage and explicitly test every safety-critical branch; local PS7 and PS5.1 each measured `83.87%` of `1,488` analyzed commands with no skipped tests at `6d53f38`, while hosted CI and remaining unexecuted failure branches still require evidence.
