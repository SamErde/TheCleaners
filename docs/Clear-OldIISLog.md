...
An absent optional registry setting is normal. Registry access or provider failures are reported through the error stream and honor `-ErrorAction Stop`. With continuing error handling, successful summaries describe only roots that were successfully discovered and enumerated, while `DiscoveryFailed` summaries identify roots that failed validation, metadata checks, or enumeration; the combined output is not a complete inventory.
