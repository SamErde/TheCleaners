...
PR runs remain candidate evidence only. The exact merged results above do not automatically cover a later `main` commit. TC-008 as a whole remains open for protected publication, published clean installation, and maintainer release acceptance. The baseline risk review is complete; its remaining native/platform/product cases belong to deferred labs. Windows/IIS/Exchange/profile lab validation is future work, not underway; IIS and Exchange retain their explicit-WhatIf preview locks. Follow the [non-lab release sequence](release-plan-1.0.md#current-non-lab-delivery-sequence) and [publishing controls](publishing.md) for environment configuration, credential setup, local rehearsal and a separately approved prerelease.
