...
- Documentation verification foundations: source/run-bound content manifests, restricted HTTP transport and isolated regression tests. Verify exact file sets, lengths and SHA-256 digests; permit HTTPS, or test-only HTTP at `127.0.0.1`/`localhost`, without following redirects. Deployment integration and public hosting verification remain separate work in PR #37; lab validation is deferred future work, not underway.
