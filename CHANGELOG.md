...
- Documentation verification foundations: source/run-bound content manifests, restricted HTTP transport and isolated regression tests. Verify exact file sets, lengths and SHA-256 digests; permit HTTPS with a loopback-only HTTP test exception and no redirect following. Deployment integration and public hosting verification remain separate work in PR #37; lab validation is deferred future work, not underway.
